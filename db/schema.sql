...
-- Row Level Security (enable per-table, policies added via Supabase dashboard)
...
-- Service-role bypasses RLS, so API functions work without extra policies.
-- For the admin dashboard, add a policy for authenticated users if needed:
