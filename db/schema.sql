...
-- Allow authenticated uploads to client folders
CREATE POLICY "Allow uploads to client folders" ON storage.objects
  FOR INSERT
  WITH CHECK (bucket_id = 'clients');

CREATE POLICY "Allow read from client folders" ON storage.objects
  FOR SELECT
  USING (bucket_id = 'clients');

-- ============================================================
-- ROW LEVEL SECURITY (enable on all tables, allow service role full access)
-- ============================================================
