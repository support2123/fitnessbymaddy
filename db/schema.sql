...
CREATE INDEX idx_checkins_client ON checkins (client_id, week_no);
...
CREATE INDEX idx_programs_client ON programs (client_id, week_no);
...
-- Enable Row Level Security (admin-only via service key)
...
-- For admin dashboard, authenticate via Supabase Auth + check role
