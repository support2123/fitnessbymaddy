...
function generateProgramPDF(client, weekNo, workoutPlan, nutritionPlan, notes) {
  return new Promise((resolve, reject) => {
    ...
    // Divider
    doc.moveTo(50, 150).lineTo(545, 150).strokeColor(BRAND.gold).lineWidth(2).stroke();
    ...
    y += 10;
    doc.moveTo(50, y).lineTo(545, y).strokeColor(BRAND.lightGrey).lineWidth(1).stroke();
    y += 20;
        ...
        for (const meal of nutritionPlan.meals) {
          ...
          if (meal.items) {
            for (const item of meal.items) {
              doc.fontSize(9).fill(BRAND.black).font('Helvetica')
                .text(`- ${item}`, 70, y);
              y += 14;
            }
          }
    ...
    if (notes) {
      ...
      doc.fontSize(12).fill(BRAND.black).font('Helvetica-Bold')
        .text('COACH NOTES', 50, y, { characterSpacing: 1 });
      y += 20;
      doc.fontSize(10).fill(BRAND.grey).font('Helvetica')
