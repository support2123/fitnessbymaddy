...
module.exports = async function handler(req, res) {
  ...
  const [
    ...
    db.from('messages').select('phone, body, sent_at').eq('template_name', 'admin_escalation').order('sent_at', { ascending: false }).limit(10),
  ]);
  ...
  for (const client of activeClientsList) {
    ...
    if (!lastCheckin || lastCheckin.week_no < currentWeek) {
      pendingList.push({
        ...
        currentWeek,
        lastCheckinWeek: lastCheckin?.week_no || 0,
      });
    }
  }
