...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (issues) {
      ...
      if (trigger) {
        await escalate(client.phone, `checkin_${trigger}`, issues);
      }
    }

    await sendWhatsApp(client.phone, 'checkin_confirmed', [
      client.name || 'there',
      `Week ${week_no}`,
      'Your check-in has been received! Your updated program will be sent soon.'
    ]);
