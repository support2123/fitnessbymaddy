  ...
  try {
    ...
    if (issues) {
      ...
      if (escalation.needed) {
        await notifyMaddy(
          client.phone,
          `Week ${week_no} check-in: ${escalation.reason}`,
          issues
        );
      }
    }
    ...
    if (isHinglish(market)) {
      await sendTemplate(client.phone, 'checkin_thanks_hi', [client.name, String(week_no)]);
    } else {
      await sendTemplate(client.phone, 'checkin_thanks_en', [client.name, String(week_no)]);
    }
