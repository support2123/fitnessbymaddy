const { getClient } = require('../lib/supabase');
const { sendTemplate, sendText } = require('../lib/whatsapp');

module.exports = async function handler(req, res) {
  if (req.method === 'OPTIONS') return res.status(200).end();
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  const supabase = getClient();

  try {
    const { phone, type, template_name, params, text, bypass_rate } = req.body;

    if (!phone) return res.status(400).json({ error: 'Missing phone' });

    const { data: lead } = await supabase
      .from('leads')
      .select('status')
      .eq('phone', phone)
      .single();

    if (lead && lead.status === 'dropped') {
      return res.status(403).json({ error: 'Lead opted out — cannot send messages' });
    }

    let result;
    if (type === 'template' && template_name) {
      result = await sendTemplate(phone, template_name, params || {}, {
        supabase,
        bypassRate: bypass_rate === true,
      });
    } else if (type === 'text' && text) {
      result = await sendText(phone, text, { supabase });
    } else {
      return res.status(400).json({ error: 'Invalid type — use "template" or "text"' });
    }

    if (result.rateLimited) {
      return res.status(429).json({ error: 'Rate limited — max 1 message per 2 hours' });
    }

    return res.status(200).json({ ok: true, result });
  } catch (err) {
    console.error('send-whatsapp error:', err.message);
    return res.status(500).json({ error: 'Internal error' });
  }
};
