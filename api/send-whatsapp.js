...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (freeform_message) {
      ...
      return res.status(400).json({ error: 'Provide template_name or freeform_message' });
    }

    if (result.rateLimited) {
      return res.status(429).json({ error: 'Rate limited', details: 'Max 1 msg per 2hrs for non-clients' });
    }
