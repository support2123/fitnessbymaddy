-- FitnessByMaddy — canonical schema
...
create index if not exists programs_flagged_idx on public.programs(flagged_for_review) where flagged_for_review = true;
...
create index if not exists escalations_unresolved_idx on public.escalations(resolved) where resolved = false;
