...
-- Enable Row Level Security (admin dashboard uses service key, so no RLS policies needed for API)
...
-- Service role bypasses RLS, so API functions work without policies
