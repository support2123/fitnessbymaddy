...
-- Intake data (extended onboarding form)
...
CREATE INDEX idx_intake_lead ON intake_data(lead_id);
...
CREATE INDEX idx_reschedules_client ON reschedules(client_id);
...
CREATE POLICY "Service role full access" ON intake_data FOR ALL USING (true);
CREATE POLICY "Service role full access" ON reschedules FOR ALL USING (true);

-- Create storage bucket for program PDFs and photos
