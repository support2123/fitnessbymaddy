...
-- Service role bypasses RLS, so API calls with service key will work.
-- For the admin dashboard, create a policy for authenticated users:
CREATE POLICY "Admin read all leads" ON leads FOR SELECT
  TO authenticated USING (true);
CREATE POLICY "Admin read all clients" ON clients FOR SELECT
  TO authenticated USING (true);
CREATE POLICY "Admin read all checkins" ON checkins FOR SELECT
  TO authenticated USING (true);
CREATE POLICY "Admin read all programs" ON programs FOR SELECT
  TO authenticated USING (true);
CREATE POLICY "Admin read all messages" ON messages FOR SELECT
  TO authenticated USING (true);
