...
-- 4. Programs table (generated weekly per client)
...
-- 6. Escalations table (for tracking human-escalation events)
...
-- Service role bypasses RLS; these policies allow authenticated admin access
CREATE POLICY "Admin full access on leads" ON leads
  FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin full access on clients" ON clients
  FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin full access on checkins" ON checkins
  FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin full access on programs" ON programs
  FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin full access on messages" ON messages
  FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin full access on escalations" ON escalations
  FOR ALL USING (auth.role() = 'authenticated');

-- 8. Storage bucket for client files
